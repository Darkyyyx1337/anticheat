...
-- Dacă funcția ServerFunc.CreateLog nu este definită, presupunem că este pentru loguri într-un canal extern
local ServerFunc = ServerFunc or {}  -- Asigurăm că ServerFunc există
ServerFunc.CreateLog = ServerFunc.CreateLog or function(data)
    -- Aceasta ar trebui să trimită logul într-un canal extern, să presupunem că este un log într-un canal Discord
    -- De exemplu, folosind un HTTP POST request pentru a trimite date către un webhook Discord
    -- Poti înlocui această funcție cu propriul tău mod de logare
end

-- Adăugăm handler-ul pentru oprirea resursei
AddEventHandler('onResourceStop', function(resourceName)
    -- Logăm mesajul de debug
    debugLog('Code: JD1006')

    -- Creăm mesajul de log pentru oprirea resursei
    local lang = {
        DefaultLogs = {
            ResourceStop = "Resursa {} s-a oprit"
        }
    }

    -- Înlocuim {} cu numele resursei
    local logMessage = lang.DefaultLogs.ResourceStop:format(resourceName)

    -- Trimitem logul către ServerFunc
    ServerFunc.CreateLog({EmbedMessage = logMessage, channel = 'resources'})
end)
